...
bool IOCPServer::CreateListenSock()
{
	...
	m_ListenSock = WSASocket(AF_INET, SOCK_STREAM, 
		IPPROTO_IP, nullptr, 0, WSA_FLAG_OVERLAPPED);
...
bool IOCPServer::CreateProcessThreads()
{
	...
	{
		hThread = (HANDLE)_beginthreadex(nullptr, 0, &CallProcessThread,
...
bool IOCPServer::CreateWorkerThreads()
{
	...
	{
		hThread = (HANDLE)_beginthreadex(nullptr, 0, &CallWorkerThread,
			this, CREATE_SUSPENDED , &uiThreadId);
		if(hThread == nullptr)
		{
			LOG( LOG_ERROR_LOW , 
...
{
	m_hWorkerIOCP = CreateIoCompletionPort(INVALID_HANDLE_VALUE, nullptr, 0, 1);
	if(m_hWorkerIOCP == nullptr)
	{
		LOG( LOG_ERROR_LOW , 
...
{
	m_hProcessIOCP = CreateIoCompletionPort(INVALID_HANDLE_VALUE, nullptr, 0, 1);
	if(m_hProcessIOCP == nullptr)
	{
		LOG( LOG_ERROR_LOW , 
...
void IOCPServer::WorkerThread()
{
	...
	Connection*			lpConnection = nullptr;
	...
	while( m_bWorkThreadFlag )
	{
		...
		{
			if( lpOverlapped == nullptr && lpConnection == nullptr )
			{
				LOG( LOG_ERROR_LOW , 
					"SYSTEM | IOCPServer::WorkerThread() | GetQueuedCompletionStatus() Failed : (%u)",
					GetLastError() );
...
void IOCPServer::ProcessThread()
{
	...
	while( m_bProcessThreadFlag )
	{
		...
		bSuccess = GetQueuedCompletionStatus(m_hProcessIOCP,
			...
			INFINITE);
		//쓰레드 종료
		if( TRUE == bSuccess && nullptr == lpConnection )
			break;
