...
namespace LogicLib
{
	...
	ERROR_SET Lobby::EnterUser(Player* pUser)
	{
		if (m_UserIndexDic.size() >= m_MaxUserCount) {
			return ERROR_SET::LOBBY_ENTER_MAX_USER_COUNT;
		}

		if (FindUser(pUser->GetIndex()) != nullptr) {
			return ERROR_SET::LOBBY_ENTER_USER_DUPLICATION;
		}
	...
	Player* Lobby::FindUser(const int userIndex)
	{
	...
	ERROR_CODE Lobby::AddUser(Player* pUser)
	{
