...
public:
	...
	//TODO 코드 이해를 하고 , 고칠수 있을게있으면 고치고
	// 안되면 새로 구현해라
	bool Create(const int CircleBufferSize)
	{
		if (CircleBufferSize == -1)
		{
			return false;
		}
	...
	char* ForwardMark(const int forwardLength, const int nextLength, const unsigned long remainLength)
	{
		SpinLockGuard Lock(&m_CS);
...
private:
	...
	int m_CircleBufferSize = -1;
	int m_UsedBufferSize = -1;
