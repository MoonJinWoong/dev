...
void IocpService::DoAccept(const CustomOverlapped* pOverlappedEx)
{
	...
	if (!pSession->GetLocalAndRemoteAddr())
	{
...
void IocpService::DoRecv(CustomOverlapped* pOver, const unsigned long ioSize)
{
...
void IocpService::RecvProcess(Session* pSession, unsigned long& remainByte, char* pBuffer)
{
	...
	while (true)
	{
		...
		if (remainByte >= (unsigned long)currentSize)
		{
...
void IocpService::EchoSend(Session *pSession, const unsigned long ioSize)
{
	WSABUF b;
